...
/**
 * @brief The GroupGraphicsObject class handles the graphical part of a node
 * group. Each node group is associated with a unique GroupGraphicsObject.
 */
class GroupGraphicsObject : public QObject, public QGraphicsRectItem {
 ...
 public:
  ...
  /**
   * @brief Moves the position of all the nodes of this group by the amount
   * given.
   * @param offset 2D vector representing the amount by which the group has
   * moved.
   */
  ...
  /**
   * TODO
   * @brief Sets the lock state of the group. Locked groups don't allow
   * individual interactions with its nodes, and can only be moved or selected
   * as a whole.
...
//  void lock(bool locked);
  ...
  /**
   * @brief Returns the lock state of the group. Locked groups don't allow
   * individual interactions with its nodes, and can only be moved or selected
   * as a whole.
   */
  bool locked() const;
   ...
   */
  void setHovered(bool hovered, QGraphicsSceneHoverEvent* event);
   ...
   */
  void setPossibleChild(NodeId possibleChild);
...
connections() const;
 ...
 public:
  ...
  /**
   * @brief _borderPen Object that dictates how the group border should be
   * drawn.
   */
  QPen _borderPen;
 ...
 private:
  /**
   * @brief _scene Reference to the scene object in which this object is
   * included.
   */
  DataFlowGraphicsScene& _scene;

  /**
   * @brief _group Reference to the group instance that corresponds to this
   * object.
   */
  NodeGroup& _group;
  ...
  /**
   * @brief _possibleChild Pointer that temporarily is set to an existing node
   * when the user drags the node to this group's area.
   */
  NodeId _possibleChild;

  /**
   * @brief _locked Holds the lock state of the group. Locked groups don't allow
   * individual interactions with its nodes, and can only be moved or selected
   * as a whole.
   */
  bool _locked;
  ...
  static constexpr QMarginsF _margins =
      QMarginsF(_groupBorderX,
