...
std::unordered_set<ConnectionId> DataFlowGraphModel::connections(
  ...
  if (it != _connectivity.end()) {
    for (auto& nodeAndPort : it->second) {
      ConnectionId conn{nodeId, portIndex, nodeAndPort.first,
                        nodeAndPort.second};
...
bool DataFlowGraphModel::connectionExists(
    ConnectionId const connectionId) const {
  auto key =
      std::make_tuple(getNodeId(PortType::Out, connectionId), PortType::Out,
...
void DataFlowGraphModel::addConnection(ConnectionId const connectionId) {
  auto connect = [&](PortType portType) {
    auto key = ConnectivityKey{getNodeId(portType, connectionId), portType,
                               getPortIndex(portType, connectionId)};

    PortType opposite = oppositePort(portType);

    _connectivity[key].insert(
...
QVariant DataFlowGraphModel::nodeData(NodeId nodeId, NodeRole role) const {
  ...
  switch (role) {
    ...
    case NodeRole::NicknameVisible: {
      result = model->nicknameVisible();
    } break;
    case NodeRole::Nickname: {
...
bool DataFlowGraphModel::setNodeData(NodeId nodeId,
                                     NodeRole role,
                                     QVariant value) {
  Q_UNUSED(nodeId);
  Q_UNUSED(role);
...
bool DataFlowGraphModel::setPortData(NodeId nodeId,
                                     PortType portType,
                                     PortIndex portIndex,
                                     PortRole role) const {
  Q_UNUSED(nodeId);
...
bool DataFlowGraphModel::deleteConnection(ConnectionId const connectionId) {
  ...
  auto disconnect = [&](PortType portType) {
    auto key = ConnectivityKey{getNodeId(portType, connectionId), portType,
                               getPortIndex(portType, connectionId)};
    auto it = _connectivity.find(key);
    ...
    for (auto const& otherSide : otherSideConnections) {
      if (std::get<1>(key) == PortType::Out) {
        connJsonArray.append(
            saveConnection(ConnectionId{std::get<0>(key), std::get<2>(key),
                                        otherSide.first, otherSide.second}));
      }
    }
  }
...
void DataFlowGraphModel::loadNode(QJsonObject const& nodeJson) {
  ...
  QString delegateModelName = nodeJson["model"].toObject()["name"].toString();
  ...
  if (model) {
    ...
    QPointF const pos(posJson["x"].toInt(), posJson["y"].toInt());
...
QJsonObject DataFlowGraphModel::saveConnection(
    ConnectionId const& connId) const {
...
void DataFlowGraphModel::loadConnection(QJsonObject const& connJson) {
