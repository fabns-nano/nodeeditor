...
FlowScene::FlowScene(std::shared_ptr<DataModelRegistry> registry,
  ...
  // Defines the sceneRect as the maximum rectangle achievable using int ranges.
  // Theoretically a QGraphicsScene could have infinite area, but the area that
  // a scroll bar can reach is in the range (INT_MIN, INT_MAX).
  QRect maximumRect{
      QPoint(std::numeric_limits<int>::lowest() / 2,
...
std::shared_ptr<Connection> FlowScene::loadConnectionToMap(
    const QJsonObject& connectionJson,
...
TypeConverter FlowScene::getConverter(const QJsonObject& connectionJson) {
...
Node& FlowScene::restoreNode(QJsonObject const& nodeJson, bool keep_id) {
  return loadNodeToMap(nodeJson, _nodes, keep_id);
}

Node& FlowScene::loadNodeToMap(
    const QJsonObject& nodeJson,
...
std::weak_ptr<NodeGroup> FlowScene::createGroup(std::vector<Node*>& nodes,
                                                QString groupName) {
  if (nodes.empty())
  ...
  auto group =
      std::make_shared<NodeGroup>(nodes, QUuid::createUuid(), groupName, this);
...
std::weak_ptr<QtNodes::NodeGroup> FlowScene::createGroupFromSelection(
    QString groupName) {
  auto nodes = selectedNodes();
  return createGroup(nodes, groupName);
}

std::vector<std::shared_ptr<Connection>> FlowScene::connectionsWithinGroup(
    const QUuid& groupID) {
...
FlowScene::restoreGroup(QJsonObject const& groupJson) {
  // since the new nodes will have the same IDs as in the file and the
  // connections need these old IDs to be restored, we must create new IDs and
  // map them to the old ones so the connections are properly restored
  ...
  return std::make_pair(
      createGroup(group_children, groupJson["name"].toString()), IDsMap);
}

void FlowScene::removeGroup(const QUuid& groupID) {
...
void FlowScene::addNodeToGroup(const QUuid& nodeID, const QUuid& groupID) {
  auto group = _groups.at(groupID);
...
void FlowScene::removeNodeFromGroup(const QUuid& nodeID) {
...
void FlowScene::iterateOverNodeDataDependentOrder(
  ...
  auto areNodeInputsVisitedBefore = [&](Node const& node,
    ...
    for (size_t i = 0; i < model.nPorts(PortType::In); ++i) {
      auto connections =
          node.nodeState().connections(PortType::In, static_cast<PortIndex>(i));
...
std::unordered_map<QUuid, std::shared_ptr<NodeGroup>> const& FlowScene::groups()
    const {
  return _groups;
}
...
std::vector<Node*> FlowScene::selectedNodes() const {
  ...
  for (QGraphicsItem* item : graphicsItems) {
    ...
    } else if (auto* ggo = qgraphicsitem_cast<GroupGraphicsObject*>(item);
               ggo) {
...
void FlowScene::save(const QString& fileName) const {
  QFile file(fileName);
...
QString FlowScene::load() {
  QString fileName = QFileDialog::getOpenFileName(
      nullptr, tr("Open Flow Scene"), QDir::currentPath(),
      tr("Flow Scene Files (*.flow)"));

  return this->load(fileName);
}

QString FlowScene::load(const QString& fileName) {
...
std::unordered_map<QUuid, QUuid> FlowScene::loadFromMemory(
...
QByteArray FlowScene::saveItems(const QList<QGraphicsItem*>& items) const {
  ...
  for (auto* item : items) {
    if (auto* cgo = qgraphicsitem_cast<ConnectionGraphicsObject*>(item)) {
      ...
      if (!connectionJson.isEmpty() && savedNodeIDs.count(inID) != 0 &&
...
QByteArray FlowScene::saveItems(QGraphicsItem* item) const {
  QList<QGraphicsItem*> dummyList({item});
  return saveItems(dummyList);
}

std::unordered_map<QUuid, QUuid> FlowScene::loadItems(const QByteArray& data,
                                                      QPointF pastePos,
                                                      bool usePastePos) {
...
void FlowScene::saveGroupFile(const QUuid& groupID) {
  QString fileName = QFileDialog::getSaveFileName(
      nullptr, tr("Save Node Group"), QDir::homePath(),
...
std::weak_ptr<NodeGroup> FlowScene::loadGroupFile() {
  QString fileName = QFileDialog::getOpenFileName(
      nullptr, tr("Open Node Group"), QDir::currentPath(),
      tr("Node Group files (*.group)"));
...
void FlowScene::nodePortsChanged(const QUuid& nodeId,
...
void FlowScene::insertNodePort(const QUuid& nodeId,
                               const QtNodes::PortType portType,
                               size_t index) {
  auto nodeIt = _nodes.find(nodeId);
  if (nodeIt != _nodes.end()) {
...
void FlowScene::eraseNodePort(const QUuid& nodeId,
                              const QtNodes::PortType portType,
                              size_t index) {
  auto nodeIt = _nodes.find(nodeId);
  if (nodeIt != _nodes.end()) {
...
Node* locateNodeAt(QPointF scenePoint,
                   FlowScene& scene,
                   QTransform const& viewTransform) {
  // items under cursor
  QList<QGraphicsItem*> items = scene.items(scenePoint, Qt::IntersectsItemShape,
