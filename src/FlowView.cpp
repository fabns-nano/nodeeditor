...
QAction* FlowView::copySelectionAction() const {
  return _copySelectionAction;
}

QAction* FlowView::cutSelectionAction() const {
  return _cutSelectionAction;
}

QAction* FlowView::pasteClipboardAction() const {
  return _pasteClipboardAction;
}

QAction* FlowView::createGroupFromSelectionAction() const {
  return _createGroupFromSelectionAction;
}

QAction* FlowView::loadGroupAction() const {
  return _loadGroupAction;
}

QByteArray FlowView::mimeToJson(const QMimeData* mimeData) const {
  if (mimeData != nullptr) {
...
void FlowView::setScene(FlowScene* scene) {
  ...
  connect(_copySelectionAction, &QAction::triggered, this,
          &FlowView::copySelectionToClipboard);
  ...
  connect(_cutSelectionAction, &QAction::triggered, this,
          &FlowView::cutSelectionToClipboard);
  ...
  connect(_createGroupFromSelectionAction, &QAction::triggered,
          [this]() { _scene->createGroupFromSelection(); });
  ...
  connect(_loadGroupAction, &QAction::triggered, this,
          &FlowView::handleLoadGroup);
  addAction(_loadGroupAction);
}

void FlowView::groupContextMenu(QContextMenuEvent* event,
  ...
  connect(saveGroupAction, &QAction::triggered, [&ggo, &_scene = _scene]() {
...
void FlowView::nodeContextMenu(QContextMenuEvent* event,
  ...
  if (ngo->node().isInGroup()) {
    if (auto nodeGroup = ngo->node().nodeGroup().lock(); nodeGroup) {
      ...
      removeFromGroupAction->setText(
          QString("Remove from \"" + nodeGroup->name()) + "\"");
      nodeMenu.addAction(removeFromGroupAction);

      connect(removeFromGroupAction, &QAction::triggered,
              [this, &ngo] { _scene->removeNodeFromGroup(ngo->node().id()); });
    }
  } else {
    ...
    for (const auto& groupEntry : _scene->groups()) {
      ...
      connect(currentGroupAction, &QAction::triggered,
              [&_scene = _scene, groupEntry, &ngo] {
                _scene->addNodeToGroup(ngo->node().id(),
                                       groupEntry.second->id());
      ...
      connect(currentGroupAction, &QAction::hovered,
              [groupEntry, &_scene = _scene]() {
                for (const auto& group : _scene->groups()) {
                  group.second->groupGraphicsObject().setHovered(group ==
                                                                 groupEntry);
                }
              });
...
void FlowView::copySelectionToClipboard() {
  ...
  _pasteClipboardAction->setEnabled(
      !clipboardData->data(_clipboardMimeType).isEmpty());
}
...
void FlowView::pasteFromClipboard() {
  ...
  // if the paste action comes from a defined position (e.g. right-click context
  // menu)
...
void FlowView::handleFilePaste(const QString& filepath, const QPointF& pos) {
...
void FlowView::loadFilesFromMime(const QMimeData* mimeData,
                                 const QPointF& pos) {
  if (mimeData->hasUrls()) {
...
bool FlowView::checkMimeFiles(const QMimeData* mimeData) const {
  if (mimeData->hasUrls()) {
    auto urls = mimeData->urls();
    for (const auto& url : urls) {
...
void FlowView::gentleZoom(double factor) {
...
void FlowView::clipCurrentScale() {
...
void FlowView::contextMenuEvent(QContextMenuEvent* event) {
  ...
  if (clickedItem) {
    ...
    if (auto groupGO = qgraphicsitem_cast<GroupGraphicsObject*>(clickedItem);
        groupGO)
      groupContextMenu(event, groupGO);
    else if (auto nodeGO = qgraphicsitem_cast<NodeGraphicsObject*>(clickedItem);
             nodeGO)
...
void FlowView::mousePressEvent(QMouseEvent* event) {
  ...
  if (event->button() == Qt::LeftButton) {
    ...
    if (selectedItem != nullptr && selectedItem->isEnabled() &&
