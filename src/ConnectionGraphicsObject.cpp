...
void ConnectionGraphicsObject::initializePosition() {
  ...
  if (_connectionState.requiredPort() != PortType::None) {
    ...
    if (ngo) {
      ...
      QPointF pos = geometry.portScenePosition(nodeId, attachedPort, portIndex,
                                               nodeSceneTransform);
...
void ConnectionGraphicsObject::setEndPoint(PortType portType,
                                           QPointF const& point) {
...
std::pair<QPointF, QPointF> ConnectionGraphicsObject::pointsC1C2Vertical()
    const {
  double const defaultOffset = 200;
